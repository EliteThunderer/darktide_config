...
AUTOMATED_CACHE_DIR = os.path.join(os.environ.get(
    "APPDATA", ""), "DarktideSettingsExporter")
...
def get_game_directory_and_save_path():
    ...
    game_dir = filedialog.askdirectory(
        title="1/2: Select the Darktide folder containing your user_settings.config file")
    if not game_dir:
    ...
    save_destination_dir = filedialog.askdirectory(
        title="2/2: Select the folder your future mod_settings files should be saved to")
...
def write_new_shareable_settings_file(output_filename, target_header):
    ...
    if not os.path.exists(config_path):
        print(
            f"Error: 'user_settings.config' file not found inside selected folder: {config_folder}.")
        print(
            f"To fix this, delete the folder at: {AUTOMATED_CACHE_DIR} and run again.")
        return False

    with open(config_path, "r", encoding="utf-8") as f:
        data = sjson.loads(f.read())
        try:
            values = data[target_header]
            with open(output_filename, "w", encoding="utf-8") as f:
                f.write("-- EXPORTED_MOD_LIST_START\n")
                for key in sorted(values.keys(), key=str.lower):
                    f.write(f"-- MOD: {key}\n")
                f.write("-- EXPORTED_MOD_LIST_END\n\n")
                f.write(sjson.dumps(values))
        except KeyError:
            print(f"ERROR: {target_header} not found in {config_path}.")

    print(f"Successfully exported {target_header} to: {output_filename}")
    return True


if __name__ == "__main__":
    write_new_shareable_settings_file(
        output_filename="mod_settings_config.txt",
